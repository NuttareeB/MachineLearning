...
def preprocess(data, is_train):
    ...
    bed_bath_ratio = data["bedrooms"]/data["bathrooms"]
    pre_data = pd.concat([data["dummy"], normalized_data, bin_data, bed_bath_ratio], axis=1)
...
start = time.time()
t_costs, d_costs = fit(data, train_data["price"], devdata, dev_data["price"], 0.1, 0.05)
