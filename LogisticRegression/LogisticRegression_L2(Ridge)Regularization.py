...
def loss_fn(w, x, y, regp=0.5):
    ...
    n = len(y)
    return ((1/n) * np.sum(err)) + (regp*(w[:-1].T @ w[:-1]))
...
start = time.time()
t_costs, d_costs = fit(traindata, traindata_label["Response"], devdata, devdata_label["Response"], 0.1, 0.001)
